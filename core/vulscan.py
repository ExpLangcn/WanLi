...
def pocscan(domain):
    if config['systeam'] == 'mac':
        console.print("[[bold red]WanLi Scan[/bold red]] [[bold green]+[/bold green]] [bold red]Log[/bold red] >  [bold yellow]正在启动Nuclei漏扫节点ing.........[/bold yellow]")
        time.sleep(3)
        console.print("[[bold red]WanLi Scan[/bold red]] [[bold green]+[/bold green]] [bold green]Nuclei[/bold green] >  [bold purple]正在使用Nuclei漏扫节点进行漏洞扫描...[/bold purple]")
        date = str(d.datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
    ...
    if config['systeam'] == 'windows':
        ...
        console.print("[[bold red]WanLi Scan[/bold red]] [[bold green]+[/bold green]] [bold green]Nuclei[/bold green] >  [bold purple]正在使用Nuclei漏扫节点进行漏洞扫描...[/bold purple]")
        date = str(d.datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
        os.system("core\\plus\\windows\\nuclei.exe -u %s -t ./core/nuclei-templates/ -s %s -silent -stats -si 10 -retries 2 -me %s"%(domain, config['severity'], config['Output'] + '/md/' + date + "_pocscan.md"))
