...
def dirscan(url):
    ...
    system(config.config.PYTHON + " " +config.config.dirsearch +r" -u " + url +r" -r -t 150 --recursion-status=200,301,302 --exclude-sizes=3KB -q --format csv -o logs/" + datatime + r"_dirscan_output.json")
...
def ddir(domain):
    ...
    system(config.config.PYTHON + " " + config.config.dirsearch +r" -l " + r"logs/" + datatime + r"_domain_output.txt -r -t 150 --recursion-status=200,301,302 --exclude-sizes=3KB -q --format csv -o logs/" + datatime + r"_ddir_output.csv"))
