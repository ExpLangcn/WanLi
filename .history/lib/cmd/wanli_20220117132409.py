...
def g_fofascan(target):
    if config.config.email == '':
        ...
        datatime = datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
        system(config.config.fofa + r" -m " + str(Email) + r" -k " + str(Key) + r" -q " + r'"' + target + r'"' + r" + r" -o logs/FOFA_" + datatime + r"_" + str(target) +r".txt")
    else:
        datatime = datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
        system(config.config.fofa + r" -q " + str(target) + r" -o logs/FOFA_" + datatime + r"_" + str(target) +r".txt")