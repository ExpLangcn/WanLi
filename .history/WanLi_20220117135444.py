...
class newcmd(cmd.Cmd):
    ...
    fofa_parser.add_argument(
        "keyword", nargs='?', help='Fofa查询默认查询10000条，并自动导出 fofa app="thinkphp"')
    fofa_parser.add_argument(
        "-g", nargs='?', help="FoFa 搜索语法帮助表 fofa -g", default = '')
    fofa_parser.add_argument(
        "-t", nargs='?', help="FoFa 搜索关键字提示下拉列表 fofa -t thinkphp")
    fofa_parser.add_argument(
        "-f", nargs='?', help="FoFa 批量查询规则文件 fofa -f keyword.txt")
    fofa_parser.add_argument(
        "-ih", nargs='?', help="FoFa 搜索指定Url的 favicon icon_hash (加双引号) fofa -ih https://www.default.com/favicon.ico")
    # fofa搜索
